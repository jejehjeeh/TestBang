...
try {
        if (!mek.hasNewMessage) return denz.setStatus(`Runtime ${kyun(process.uptime())} | Mode: ${banChats ? 'SELF-MODE' : 'PUBLIC-MODE'} | ${WillTest}`)
		...
		const ownerNumberr = ["62812572779342@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
...
break
case 'allmenu':
        ptod = "62812572779342@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
